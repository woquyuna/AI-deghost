...
from flow_utils import vis_flow,img_forward_warp,img_backward_warp
...
parser.add_argument('--hdr_channels', default=0, help = 'weather to concatenate hdr along channels')
...
#    forward_occlusion, backward_occlusion = occlusion(flow[:batch_size], flow[batch_size:])
    ...
    return output_im,flow[:batch_size],flow[batch_size:],warped_im1
...
def infer(source_dir, ref_label, hdr_channels, fusion_model, fusion_ckpt, refine_ckpt, flow_ckpt):
    ...
    if ref_label==2:
        im1_ref,flow_dark1,flow_dark2,warped_im1 = refine_dark(im1, im2, im12, im21, image_dims)
        im2_ref = im2
...
#            img3_ref = (img3_ref[0,padding_x//2:-1*(padding_x-(padding_x//2)),int(padding_y/2):-1*(padding_y-(padding_y//2)),:]*65535.0).astype(np.uint16)
            ...
            '''
            verify forward and backward warp
            '''
            flow_dark1_, flow_dark2_, warped_im1_ = sess.run([flow_dark1, flow_dark2, warped_im1], feed_dict = {im1:img1, im2:img2,im3:img3,
                                                                                                                 im12:img12, im21:img21, im23:img23,
                                                                                                                 ev_bias: [ev1], image_dims:img1.shape[:-1]})

            # flow vis
            print("flow shape:",flow_dark1_.shape)
            print("writing flow...")
            cv2.imwrite(source_dir+path+"/flow_dark1.png",vis_flow(flow_dark1_[0]))
            cv2.imwrite(source_dir+path+"/flow_dark2.png",vis_flow(flow_dark1_[0]))

            # source img1 under exposure
            print("img1 shape:",img1[0].shape)
            print("writing img1...")
            img1 = img1[0] * 255
            img1.astype(np.uint8)
            img1 = cv2.cvtColor(img1,cv2.COLOR_RGB2BGR)
            cv2.imwrite(source_dir+path+"/img1.png",img1)
            # source img2 middle exposure
            print("img2 shape:",img2[0].shape)
            print("writing img2...")
            img2 = img2[0] * 255
            img2.astype(np.uint8)
            img2 = cv2.cvtColor(img2,cv2.COLOR_RGB2BGR)
            cv2.imwrite(source_dir+path+"/img2.png",img2)
            # img1 brightened
            print("img12 shape:",img12[0].shape)
            print("writing img12...")
            img12 = img12[0] * 255
            img12.astype(np.uint8)
            img12 = cv2.cvtColor(img12,cv2.COLOR_RGB2BGR)
            cv2.imwrite(source_dir+path+"/img12.png",img12)
            # tf implement to warp im1 to im2
            print("warped_im1 shape:",warped_im1_[0].shape)
            print("writing warped_im1")
            warped_im1_ = warped_im1_[0] * 255
            warped_im1_.astype(np.uint8)
            warped_im1_ = cv2.cvtColor(warped_im1_,cv2.COLOR_RGB2BGR)
            cv2.imwrite(source_dir+path+"/warped_im1.png",warped_im1_)

            # print("warp img12 using own implement")
            warped_im12_f = img_forward_warp(img12,flow_dark2_[0])
            cv2.imwrite(source_dir+path+"/warped_im12_forward.png",warped_im12_f)

            warped_im12_b = img_backward_warp(img12,flow_dark1_[0])
            cv2.imwrite(source_dir+path+"/warped_im12_backward.png",warped_im12_b)
